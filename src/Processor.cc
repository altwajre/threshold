...
void Processor::doThreshold() {
    ...
    for (auto &el : neighbourStats) {
        ...
        if (ratio > 0.67) {
            ...
            emit(sigNeighbourThresholded,
                    static_cast<cSimpleModule*>(gate(el.first)->getPathStartGate()->getOwner()->getOwner())->par(
                            "address").intValue());
        }
        el.second.count = 0;
    }
    ...
    for (auto const& el : neighboursThresholded) {
        EV
                  << static_cast<cSimpleModule*>(gate(el)->getPathStartGate()->getOwner()->getOwner())->par(
                          "address").intValue() << ", ";
    }
    EV << endl;
}
